...
class ContainerState(State):
    ...
    def run(self):
        print "Start container state with id %s" % self._id

    def enter(self):
        print "Calling enter() script of container state with id %s" % self._id

    def exit(self, transition):
        print "Calling exit() script of container state with id %s" % self._id

    def get_inputs_for_state(self, state):
        print "Return data inputs for container state with id %s" % self._id

    def get_outputs_for_state(self, state):
        print "Return data outputs for container state with id %s" % self._id

    def get_transition_for_outcome(self, state, outcome):
        print "Return transition for a specific child state and its specific outcome"

    def get_target_state_for_transition(self, transition):
        print "Return state for specific transition"