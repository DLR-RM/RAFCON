...
class StateOutcomesTreeController(Controller):
    ...
    def __init__(self, model, view):
        ...
        self.to_state_combo_dict = gtk.ListStore(gobject.TYPE_STRING, gobject.TYPE_STRING, gobject.TYPE_STRING)
        self.to_outcome_combo_dict = gtk.ListStore(gobject.TYPE_STRING, gobject.TYPE_STRING, gobject.TYPE_STRING)
        ...
        if hasattr(model, 'parent') and model.parent is not None:
            ...
            for smdl in model.parent.states.values():
                if not model.state.state_id == smdl.state.state_id:
                    self.to_state_combo_dict.append([smdl.state.name, smdl.state.state_id, parent_id])
            ...
            for outcome in model.parent.state.outcomes.values():
                ...
                self.to_outcome_combo_dict.append([outcome.name, outcome.outcome_id, parent_id])
    ...
    def register_view(self, view):
        ...
        def cell_text(column, cell_renderer, model, iter, container_model):
            ...
            if column.get_title() == 'ID':
                ...
                cell_renderer.set_property("editable", True)
                cell_renderer.set_property("model", self.to_state_combo_dict)
                ...
                cell_renderer.set_property("editable", True)
                cell_renderer.set_property("model", self.to_outcome_combo_dict)
                cell_renderer.set_property("text-column", 0)
                cell_renderer.set_property("has-entry", False)
        ...
        for outcome in self.model.state.outcomes.values():
            ...
            self.tree_store.append(None, [outcome.outcome_id, outcome.name, to_state, to_outcome, '#f0E5C7', '#f0E5c7', outcome, self.model.state])
