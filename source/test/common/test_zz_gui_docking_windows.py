...
def restore_original_runtime_config_file():
    path = os.path.join(os.path.expanduser('~'), '.config', 'rafcon')
    temp_path = os.path.join(path, 'temp')
    if os.path.isdir(temp_path):
        os.remove(os.path.join(path, 'runtime_config.yaml'))
        os.rename(os.path.join(temp_path, 'runtime_config.yaml'), os.path.join(path, 'runtime_config.yaml'))
        os.rmdir(temp_path)
...
def trigger_docking_signals(*args):
    ...
    main_window_controller.view.left_bar_window.get_top_widget().resize(300, 600)
    time.sleep(1)
    main_window_controller.view.left_bar_window.get_top_widget().move(100, 100)
    time.sleep(1)
    ...
    main_window_controller.view.right_bar_window.get_top_widget().resize(300, 600)
    time.sleep(1)
    main_window_controller.view.right_bar_window.get_top_widget().move(100, 100)
    time.sleep(1)
    ...
    main_window_controller.view.console_window.get_top_widget().resize(600, 300)
    time.sleep(1)
    main_window_controller.view.console_window.get_top_widget().move(100, 100)
    time.sleep(1)
...
def test_window_positions(caplog):
    ...
    restore_original_runtime_config_file()
