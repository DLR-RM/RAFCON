...
class HierarchyState(ContainerState):
        ...
        try:
            while self.child_state is not self:
                ...
                if not self.backward_execution:  # only add history item if it is not a backward execution
                    self.execution_history.add_call_history_item(self.child_state, CallType.EXECUTE, self)

                if self.last_error is not None:
                    self.child_state.input_data['error'] = self.last_error
                self.last_error = None
                if self.last_child:
                ...
                if self.preempted:
                    if self.backward_execution:
                        ...
                        # as preemtion behaviour in backward mode is not defined, set the state to forward mode
    ...
    def _handle_forward_execution_after_child_execution(self):
        ...
        self.execution_history.add_return_history_item(self.child_state, CallType.EXECUTE, self)
