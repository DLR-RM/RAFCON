...
class LinkageOverviewController(ExtendedController, Model):
    def __init__(self, model, view):
        ...
        self.add_controller('scoped_variables', ScopedVariableListController(_model, view.scope_view))
        self.add_controller('outcomes', StateOutcomesEditorController(model, view.outcomes_view))

        if isinstance(self.model.state, LibraryState) and not isinstance(self.model.state_copy, ContainerState) or \
                isinstance(self.model.state, ExecutionState):
