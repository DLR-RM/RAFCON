...
class DebugConsoleView(View):
    def __init__(self):
        super().__init__(builder_filename=glade.get_glade_path('debug_console.ui'), parent='console_container')
        self._lock = threading.Lock()
        ######################################################
        # Logging text view
        ######################################################
        self.logging_console_view = LoggingConsoleView()
        self.logging_console_view.get_parent_widget().set_vexpand(True)
        self['console'].append(self.logging_console_view.get_parent_widget())
        ...
        self['undock_console_button'].set_child(gui_helper_label.create_button_label(constants.BUTTON_UNDOCK))
        self['undock_console_button'].set_tooltip_text("Undock debug console widget")
        self['console_hide_button'].set_child(gui_helper_label.create_button_label(constants.BUTTON_DOWNA))
