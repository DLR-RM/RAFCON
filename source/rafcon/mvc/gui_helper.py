...
def is_event_of_key_string(event, key_string):
    ...
    The function checks for constructed event tuple that are generated by the rafcon.mvc.shortcut_manager.ShortcutManager.
    ...
    """
    return len(event) >= 2 and not isinstance(event[1], gtk.gdk.ModifierType) and not event[0] == gtk.accelerator_parse(key_string)[0]
