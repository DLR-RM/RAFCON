...
def test_concurrency_barrier_save_load(caplog):
    ...
    try:
        ...
        keys.sort()
        # start item
        assert ss[keys[0]]['item_type'] == 'StateMachineStartItem'
        # container call
        assert ss[keys[1]]['item_type'] == 'CallItem'
        assert ss[keys[1]]['call_type'] == 'CONTAINER'
        assert ss[keys[1]]['state_type'] == 'BarrierConcurrencyState'
        assert json.loads(ss[keys[1]]['input_output_data']) == {'input_data_port1': 0.1, 'input_data_port2': 0.1}
        assert ss[keys[1]]['prev_hist_item_id'] == keys[0]

        # serialization of error outputs
        for i in range(2,8):
            if ss[keys[i]]['state_name'] == 'SecondState' and ss[keys[i]]['item_type'] == 'ReturnItem':
                assert json.loads(ss[keys[i]]['input_output_data']) == {
                    "output_data_port1": 10.0,
