...
BARRIER_CONCURRENCY    = TASK_1 + "/LZLXFW/KEQYNU"   # all branches must finish before moving on
PREEMPTIVE_CONCURRENCY = TASK_1 + "/LZLXFW/AESIQF"   # fastest branch cancels the rest

# inside task_2 (EAWGCM is the HierarchyState wrapping the library)
NESTED_LIBRARY      = TASK_2 + "/EAWGCM/SFSEDE/MQRVHE/FLOSSL"  # hierarchy_4, 3 libs deep
...
def _replay_from(sm, target_path):
    ...
    call_item = next(
        (
            item
            for item in run_1
            if isinstance(item, CallItem)
            and item.call_type_str == "EXECUTE"
            and item.path == target_path
        ),
        None,
    )
