...
def store_state_elements(state, state_m):
    ...
    for p_id, p in list(state.input_data_ports.items()):
    ...
    for p_id, p in list(state.output_data_ports.items()):
    ...
    for oc_id, oc, in list(state.outcomes.items()):
    ...
    if isinstance(state, ContainerState):
        ...
        for s_id, s in list(state.states.items()):
            state_elements['states'].append(s_id)
        # - check if the right models are there and only those
        model_id_store = []
        # state_m_elements['states_meta'] = {}
        for s_m_id, s_m in list(state_m.states.items()):
...
def test_add_remove_models(caplog):
    ...
    def print_all_states_with_path_and_name(state):
        ...
        if isinstance(state, ContainerState):
            for s_id, child_state in list(state.states.items()):
                state_dict['states'][s_id] = print_all_states_with_path_and_name(child_state)
            for dp_id, dp in list(state.input_data_ports.items()):
                state_dict['input_data_ports'][dp_id] = dp
            for dp_id, dp in list(state.output_data_ports.items()):
                state_dict['output_data_ports'][dp_id] = dp
            for dp_id, dp in list(state.scoped_variables.items()):
    ...
    def check_if_all_states_there(state, state_dict):
        ...
        for s_id, s_dict in list(state_dict['states'].items()):
