...
# ----------- intergration tests ----------------
sm_path_autoscroll = os.path.join(
    testing_utils.TEST_ASSETS_PATH, "unit_test_state_machines", "autoscroll_test"
)
...
def _center_state_in_view(view, state_v):
    ...
    i_x, i_y = view.get_matrix_i2v(state_v).transform_point(
        state_v.width / 2, state_v.height / 2
    )
...
def _item_drag_into_autoscroll_and_stop(
    gui, view, state_v, monkeypatch, stop_condition="BUTTON_RELEASE"
):
    from gi.repository import Gdk
    from rafcon.gui.mygaphas.tools import MoveItemTool
    ...
    monkeypatch.setattr(
        "rafcon.gui.mygaphas.guide.GuidedStateMixin.MARGIN", 0
    )  # disable snapping

    gui(view.unselect_all)
    gui(view.select_item, state_v)
    testing_utils.wait_for_gui()
    ...
    if stop_condition == "BUTTON_RELEASE":
        stop_event = _make_event(
            Gdk.EventType.BUTTON_RELEASE, move_event.x, move_event.y
        )
...
def _handle_drag_into_autoscroll_and_stop(
    gui, view, state_v, monkeypatch, stop_condition="BUTTON_RELEASE"
):
    from gi.repository import Gdk
    ...
    monkeypatch.setattr(
        tool, "grabbed_item", lambda: state_v
    )  # deterministic item grab
...
def _connection_drag_into_autoscroll_and_stop(
    gui, view, state_v, monkeypatch, stop_condition="BUTTON_RELEASE"
):
    from gi.repository import Gdk
...
def test_autoscroll_item_release_button(gui, monkeypatch):
    ...
    x, y = _center_state_in_view(view=view, state_v=state_v)
    _assert_centered(view, state_v)
    _zoom_into_view(gui, view, x, y)
    # release button in autoscroll zone to stop autoscroll
    _item_drag_into_autoscroll_and_stop(
        gui, view, state_v, monkeypatch, "BUTTON_RELEASE"
    )
...
def test_autoscroll_handle_release_button(gui, monkeypatch):
    ...
    _assert_centered(view, state_v)
    _zoom_into_view(gui, view, x, y)
    _handle_drag_into_autoscroll_and_stop(
        gui, view, state_v, monkeypatch, "BUTTON_RELEASE"
    )
...
def test_autoscroll_connection_release_button(gui, monkeypatch):
    ...
    _assert_centered(view, state_v)
    _zoom_into_view(gui, view, x, y)
    _connection_drag_into_autoscroll_and_stop(
        gui, view, state_v, monkeypatch, "BUTTON_RELEASE"
    )
...
def test_autoscroll_connection_drag_back(gui, monkeypatch):
    ...
    _zoom_into_view(gui, view, x, y)
    _connection_drag_into_autoscroll_and_stop(
        gui, view, state_v, monkeypatch, "DRAG_BACK"
    )
